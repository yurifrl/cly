...
### Added
- `modules/omp/summary`: `cly omp summary` — headless daemon that keeps the active surface's summary card fresh in the omp-summary cmux custom sidebar (the Bubbletea TUI is gone). Discovery joins `cmux --id-format both tree --json` surfaces with `cmux sessions list --agent omp --all --json` hook records; when the frontmost surface has no live omp session (the sidebar itself, shell tabs, browser panes) it falls back to the freshest live sibling session in the same workspace, so the card keeps updating while the user stares at the sidebar. The transcript digest tails the last messages (byte/line-capped, partial-line safe) extracting turns and the last exit code, fingerprints the transcript by size+mtime, and re-summarizes only on change. Summaries run through the configured AI provider (prompt v2) and persist as state v2 (v1 migrated in place, atomic tmp+fsync+rename, 0644). Each tick the rendered card — last command (▸), AI status (◆), model, stamp; empty segments omitted — is pushed unconditionally into the workspace description slot via `cmux workspace-action --action set-description` (pkg/cmux `SetDescription`), guarded against strictly newer state entries. `--once` runs a single discovery+summary pass and exits.
- `pkg/cmux`: `Tree`, `SessionsList`, `FocusPane`, `SetDescription` — tolerant JSON client over the cmux tree, hook-session listings, and workspace description writes (nullable `surface_id` as pointer, top-level `active` as its distinct flat shape), with tests over real-shaped fixtures.
