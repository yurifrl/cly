...
func installFish(root *cobra.Command, dir, confDir string) error {
	...
	// Write envs loader to conf.d so environment variables are loaded at startup
	if confDir != "" {
		if err := os.MkdirAll(confDir, 0755); err != nil {
			return fmt.Errorf("creating directory %s: %w", confDir, err)
		}
		envsContent := `# cly envs — load 1Password environment variables at shell startup.
# Generated by ` + "`cly completion fish install`" + `. Do not edit by hand.
set -l __cly_envs_cache /tmp/envs/envs.fish
if not test -f $__cly_envs_cache
    if command -q cly
        cly envs --fish >$__cly_envs_cache 2>/dev/null
    end
end
if test -f $__cly_envs_cache
    source $__cly_envs_cache
end
`
		envsPath := filepath.Join(confDir, "cly-envs.fish")
		if err := os.WriteFile(envsPath, []byte(envsContent), 0644); err != nil {
			return err
		}
		fmt.Fprintf(os.Stderr, "Installed envs loader to %s\n", envsPath)
	}
